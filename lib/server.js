...
class Server {
    ...
    _updateRequestHeaders(request, ssl) {
        ...
        if (request.headers['connection'] != 'Upgrade') delete request.headers['connection'];
    ...
    _updateResponseHeaders(response, ssl) {
        if (response.headers['connection'] != 'Upgrade') delete response.headers['connection'];
        response.headers['X-Reverse-Proxy-Version'] = require('../package.json').version;
    }
        ...
        return Promise.resolve()
            .then(() => {
                ...
                for (var host of this.hosts) {
                    ...
                    if (!_.find(host.securedByGithub.logins, l => 
                                session.user.login == l ||
                                _.find(session.orgs, o => o.login == l)
                                )) {
