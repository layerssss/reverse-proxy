...
class Server {
  constructor(options) {
    ...
    this.configWatcher = Fs.watch(
      ...
      () => {
        this.initConfig(this._options.configFilePath).catch(error =>
      ...
      .then(config => {
        ...
        for (var host of config.hosts) {
          ...
          if (host.securedByGithub) {
            ...
            Assert(host.securedByGithub.logins);
          }
  ...
  _challengeValuesFor(domain) {
    if (!this.hosts.filter(branch => branch.hostname == domain).length)
      return {};
  ...
  _updateRequestHeaders(request, ssl) {
    ...
    request.headers["X-Forwarded-Port"] =
      request.headers["X-Forwarded-Port"] || request.socket.localPort;
  ...
  _updateResponseHeaders(response, ssl) {
    if (response.headers["connection"] != "Upgrade")
      delete response.headers["connection"];
    response.headers[
      "X-Reverse-Proxy-Version"
        ...
        () =>
          new Promise((resolve, reject) => {
            if (upstreamUrl.protocol == "http:") {
              ...
              return reject(
                new Error("Unknown unstream protocol: " + upstreamUrl.protocol)
              );
            ...
            request.on("aborted", () => proxyRequest.abort());

            request.pipe(proxyRequest);
          })
      )
      .then(() => {
        this._updateResponseHeaders(proxyResponse, ssl);
      })
      .then(
        () =>
          new Promise(resolve => {
  ...
  _handleRequest(request, response, ssl) {
    ...
    return Promise.resolve().then(() => {
      ...
      for (var host of this.hosts) {
        ...
        if (!ssl) {
          if (host.allowInsecure && !host.securedByGithub)
        ...
        if (requestPath == "/" && requestQuery["code"]) {
          return Utility.requestHttpsJSON(
            ...
            return Promise.all([
              ...
              response.writeHead(302, {
                Location:
                  "https://" + requestHost + (session.redirectUrl || "/")
              });
              return;
            });
          });
        ...
        if (
          !_.find(
            host.securedByGithub.logins,
            l =>
              session.user.login == l || _.find(session.orgs, o => o.login == l)
          )
        ) {
  ...
  _handleUpgrade(request, socket, head, ssl) {
    ...
    for (var host of this.hosts) {
      ...
      if (host.securedByGithub) {
        ...
        if (!session || !session.user) return socket.end(); // stop visitor without event session
    ...
    return Promise.resolve()
      .then(() => {
        ...
        var leSniCallback = LeSni.create({
          renewWithin: 30 * 24 * 60 * 60 * 1000, // 10 days
          renewBy: 25 * 24 * 60 * 60 * 1000, // 5 days
          tlsOptions: {
